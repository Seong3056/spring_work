                ...
                <form action="${pageContext.request.contextPath}/user/join" method="post" name="joinForm">
                    ...
                    <div class="form-group">
                        ...
                        <input type="password" class="form-control" name="userPw" id="userPw"
                            placeholder="비밀번호 (영 대/소문자, 숫자 조합 8~16자 미만)">
                        <span id="msgPw"></span>
                        <!--자바스크립트에서 추가-->
                    </div>
                    <div class="form-group">
                    ...
                    <div class="mail-check-box">
                        <input id="certi" type="text" class="form-control mail-check-input" placeholder="인증번호 6자리를 입력하세요."
                            maxlength="6" disabled="disabled">
                            <button id="certiBtn" type="button" class="btn btn-primary">인증하기</button>
...
<script src="//t1.daumcdn.net/mapjsapi/bundle/postcode/prod/postcode.v2.js"></script>
    <script>
        ...
        let idFlag, pwFlag; //정규표현식 유효성 검사
    ...
    document.getElementById('idCheckBtn').onclick = function (){
        ...
        xhr.onload = () => {
            ...
            if(xhr.response == 'duplicated'){
                msgId.innerHTML = '중복된 아이디 입니다.';
                userId.style.borderColor = 'red';
            }
            else {
                msgId.innerHTML = '사용가능한 아이디 입니다.';
            }
        }
        */
        ...
        /*
        # fetch API: 자바스크립트에서 지원하는 비동기 통신 함수
        - Promise 객체를 자동으로 리턴해서 손쉽게 통신의 응답 데이터를 
          소비할 수 있게 해줍니다. (Promise: 비동기 통신의 순서를 보장하는 문법)
        - fetch 함수가 리턴하는 Promise 객체는 단순한 응답 JSON 데이터가 아닌 
          전체적이고, 포괄적인 응답정보를 가지고 있습니다.
        - 따라서, 서버가 응답한 여러정보 중 JSON데이터만 소비하려면
          json()이라는 메서드를 사용합니다.
    ...
    fetch('${pageContext.request.contextPath}/user/idCheck',reqObj)
        .then(res => res.text()) //요청완료후 응답정보에서 텍스트만빼기
            .then(data => {
                if(data == 'duplicated'){
                msgId.innerHTML = '중복된 아이디 입니다.';                
            }
            else {
                //더이상 아이디를 작성할 수 없도록 막기
    ...
    document.getElementById('mail-check-btn').onclick = () => {
        ...
        fetch('${pageContext.request.contextPath}/user/mailCheck?email=' + email)
            .then(res => res.text())
                .then(data => {
                    console.log('인증번회: ' + data);
                    document.getElementById('certi').removeAttribute('disabled');
                    document.getElementById('certiBtn').onclick = () => {
                        const num = document.getElementById('certi').value;
                        if(num === data ){
                            document.getElementById('mail-check-warn').innerHTML = '이메일 인증이 완료 되었습니다.';
                            document.getElementById('certi').setAttribute('readonly',true);
                            document.getElementById('certiBtn').setAttribute('disabled',true);
                            document.getElementById('mail-check-btn').setAttribute('disabled',true);
                        }
                        else document.getElementById('mail-check-warn').innerHTML = '인증번호가 다릅니다.';
	...
	document.querySelector('.mail-check-input').onblur = function (e) {
        ...
        //다음 주소 api 사용해 보기 (script src 추가해야 합니다.)
    ...
    document.getElementById('joinBtn').onclick = function(){
        ...
        id.onkeyup = function() {
            /*
            	...
            	특정 규칙이 잇는 문자열 집합을 규칙을 직접 지정하여 탐색하게 해주는 
            	메타 문자 입니다. 
            */
            /*test메서드를 통해 비교하여 매칭되면 true, 아니면 false 반환 */
            ...
            /* else if(regex.test(document.getElementById("userId").value == ""){
            	 document.getElementById("userId").style.borderColor = "#555";
                 document.getElementById("msgId").innerHTML = "";	
            } */
            
            else {
                document.getElementById("userId").style.borderColor = "red";
                document.getElementById("msgId").innerHTML = "옳지 않은 아이디 입니다.";
                idFlag = false;
            }
        }
        ...
        pw.onkeyup = function(){
            var regex = /^[A-Za-z0-9+]{8,16}$/;
             if(regex.test(document.getElementById("userPw").value )) {
                ...
                document.getElementById("msgPw").innerHTML = "사용가능한 비밀번호입니다.";
                pwFlag = true;
                
            } else {
                document.getElementById("userPw").style.borderColor = "red";
                document.getElementById("msgPw").innerHTML = "비밀번호를 다시 입력해주세요.";
                pwFlag = false;
            }
        }
        /*비밀번호 확인검사*/
