...
@WebAppConfiguration
public class FreooBoardControllerTest {
...
//	private FreeBoardController controller;
	...
	//MockMvc는 웹 어플리케이션을서버에 배포하지 않아도 스프링 MVC 동작을
	...
	public void setup() {
		...
		//테스트할 컨트롤러를 수동으로 주입, 하나의 컨트로럴 자체만 테스트를 진행할때 사용
//		this.mockmvc = MockMvcBuilders.standaloneSetup(controller).build();
	...
	void testList() throws Exception {
		...
		ModelAndView mv = 
		//mockmvc 동작 (mockmvc리퀘스트빌더(경로)의 결과 모델앤뷰 모델맵
		mockmvc.perform(MockMvcRequestBuilders.get("/freeboard/freeList"))
		.andReturn()
		.getModelAndView();
		
		System.out.println("Model 내 저장한 데이터: "+mv.getModelMap());
		System.out.println("응답 처리를 위해 사용할 페이지: "+ mv.getViewName());
				
	}
	
	@Test
	@DisplayName("게시글 등록 요청 처리과정")
	...
	@Test
	@DisplayName("3번 글 상세 보기 요청을 넣으면 컨트롤러는 DB에서 가지고 온 글 객체를 model에 담아서 jsp로 이동")
	...
	public void testModify() throws Exception {
		...
		String viewPage = mockmvc.perform(MockMvcRequestBuilders.post("/freeboard/modify")				
				.param("title", "테스트 수정된 새 글 제목")
				.param("content", "테스트 수정된 새 글 내용")
				.param("bno", bno))
				.andReturn().getModelAndView().getViewName();
...
//				String viewPage = mockmvc.perform(MockMvcRequestBuilders.post("/freeboard/freeDelete")
//						.param("bno", "3"))
//				.andReturn().getModelAndView().getViewName();
//				System.out.println("viewName:" + viewPage);
