...
public class ScoreController {
	...
	//@Autowired는 스프링 컨테이너에 등록되어 있는 빈들중, 해당 변수타입에 맞는
	//객체를 자동으로 변수에 주입해 주는 아노테이션 이다.
	//필드, 생성자, setter 메서드에 붙일 수 있다.
	@Autowired
	private IScoreService service;
	
	
	//점수 등록화면을 열어주는 처리를 하는 메서드
	@GetMapping("/register")
...
//		List<ScoreVO> list = service.selectAllScores();
	...
	public String selectOne(int stuId, Model model, RedirectAttributes ra) {
		...
		if(vo == null) {
			...
			model.addAttribute(service.selectOne(stuId));
	...
	@GetMapping("/delete")
	public String deleteScore(int stuId,RedirectAttributes ra) {
